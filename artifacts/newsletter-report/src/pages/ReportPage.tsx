...
function renderCustomPieLabel({
  ...
  return (
    ...
    >
      ...
      <tspan x={x} dy="1.2em">{formatNumber(String(value))}</tspan>
    </text>
  );
}
    ...
    <div className="min-h-screen bg-gray-100 p-4">
      ...
      <div className="max-w-[1200px] mx-auto mb-4 flex flex-wrap gap-2 items-center justify-between">
        <div className="flex gap-2">
          ...
          <button
            ...
            className="px-4 py-2 bg-slate-600 text-white text-sm font-medium rounded hover:bg-slate-700 transition"
          >
            Upload Logo
          </button>
      ...
      >
        ...
        <div className="bg-[#1e3a5f] text-white text-center py-3">
          <h1 className="text-xl font-bold tracking-widest uppercase">News Letter Report</h1>
        </div>
              ...
              <table className="w-full text-sm border-collapse">
                <thead>
                  <tr className="bg-[#2563eb] text-white">
                    <th className="py-2 px-3 text-left font-semibold border border-blue-400">Month</th>
                    <th className="py-2 px-3 text-right font-semibold border border-blue-400">Optin Subscribers</th>
                    <th className="py-2 px-3 text-right font-semibold border border-blue-400">Open Rate</th>
                    <th className="py-2 px-3 text-right font-semibold border border-blue-400">Click Rate</th>
                    <th className="py-2 px-3 text-center font-semibold border border-blue-400 w-10 no-print">✕</th>
                  </tr>
                </thead>
                <tbody>
                  {rows.map((row, i) => (
                    <tr key={row.id} className={i % 2 === 0 ? "bg-[#eff6ff]" : "bg-white"}>
                      <td className="py-2 px-3 border border-blue-100">
                        <input
                          ...
                          className="bg-transparent focus:outline-none border-b border-dashed border-blue-300 w-full text-sm font-medium text-[#1e3a5f]"
                        />
                      </td>
                      <td className="py-2 px-3 border border-blue-100 text-right">
                        <input
                          type="text"
                          value={row.optinSubscribers}
                          onChange={(e) => updateRow(row.id, "optinSubscribers", e.target.value)}
                          className="bg-transparent focus:outline-none border-b border-dashed border-blue-300 w-full text-right text-sm"
                        />
                      </td>
                      <td className="py-2 px-3 border border-blue-100 text-right">
                        <input
                          type="text"
                          value={row.openRate}
                          onChange={(e) => updateRow(row.id, "openRate", e.target.value)}
                          className="bg-transparent focus:outline-none border-b border-dashed border-blue-300 w-full text-right text-sm"
                        />
                      </td>
                      <td className="py-2 px-3 border border-blue-100 text-right">
                        <input
                          type="text"
                          value={row.clickRate}
                          onChange={(e) => updateRow(row.id, "clickRate", e.target.value)}
                          className="bg-transparent focus:outline-none border-b border-dashed border-blue-300 w-full text-right text-sm"
                        />
                      </td>
                      <td className="py-2 px-3 border border-blue-100 text-center no-print">
                        <button
        ...
        <div className="border-t-2 border-[#1e3a5f] mt-2">
          <div className="flex flex-wrap">
            ...
            <div className="flex-1 min-w-[300px] border-r border-gray-200 p-3">
              <h3 className="text-center text-xs font-bold uppercase tracking-wider text-[#1e3a5f] mb-2">Open Rate</h3>
              <ResponsiveContainer width="100%" height={Math.max(200, 160 + rows.length * 10)}>
                <PieChart>
                  <Pie
                    data={pieOpenData}
                    cx="50%"
                    cy="50%"
                    outerRadius={Math.min(70, 50 + rows.length * 2)}
                    dataKey="value"
                    labelLine={true}
            ...
            {/* Click Rate */}
            <div className="flex-1 min-w-[300px] border-r border-gray-200 p-3">
              <h3 className="text-center text-xs font-bold uppercase tracking-wider text-[#1e3a5f] mb-2">Click Rate</h3>
              <ResponsiveContainer width="100%" height={Math.max(200, 160 + rows.length * 10)}>
                <PieChart>
                  <Pie
                    data={pieClickData}
            ...
            <div className="flex-1 min-w-[300px] p-3">
              <h3 className="text-center text-xs font-bold uppercase tracking-wider text-[#1e3a5f] mb-2">
                Optin Subscribers
              </h3>
              <p className="text-center text-[10px] text-gray-500 mb-1">Banner ad Impressions</p>
              <ResponsiveContainer width="100%" height={Math.max(200, 160 + rows.length * 10)}>
      ...
      <p className="text-center text-xs text-gray-400 mt-3">
        Click any field to edit • Click logo area to upload • Use toolbar to add/remove months and export
      </p>
    </div>
  );
}
...
function InfoRow({
  ...
  return (
    <div className="mb-3">
      ...
      <EditableText value={value} onChange={onChange} className="text-white text-sm" multiline={multiline} />
    </div>
  );
}
...
function EditableText({
  ...
  if (multiline) {
    return (
      <textarea
        ...
        className={`bg-transparent border-b border-dashed border-blue-400 focus:outline-none focus:border-white resize-none w-full leading-snug ${className}`}
        rows={2}
      />
    );
  }
  return (
    <input
      ...
      value={value}
      onChange={(e) => onChange(e.target.value)}
      className={`bg-transparent border-b border-dashed border-blue-400 focus:outline-none focus:border-white w-full ${className}`}
    />
  );
}
